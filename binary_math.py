...
#	print(total_fraction_size)
	...
	a_size = len(operand_a_2s_comp)
	b_size = len(operand_b_2s_comp)
	current_sum = [0]*a_size
	
	for i in range(a_size-1, -1, -1):
		if (operand_b_2s_comp[i] == 1):
			addend_a = current_sum
			addend_b = operand_a_2s_comp + [0]*((a_size-1)-i)
		else:	
			addend_a = current_sum
...
#	print("".join(map(str, product_2s_comp)))
	...
	if ('.' in operand_a) or ('.' in operand_b):
...
def binary_subtraction(A, B):
	...
	operand_b_2s_comp, carry = twos_complement(operand_b_normalized)	
	difference_sum, difference_carry = binary_adder(operand_a_normalized, operand_b_2s_comp)
